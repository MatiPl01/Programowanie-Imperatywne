...
int main() {
    ...
    print_linked_list(ll_arr[0]);
    print_linked_list(ll_arr[1]);
    print_linked_list(ll_arr[2]);
...
void print_linked_list(Node *head) {
    ...
    if (curr != NULL) printf("%d", curr->val);
    curr = curr->next;
    while (curr != NULL) {
        printf(" -> %d", curr->val);
        curr = curr->next;
...
Node* merge_two_sorted_lls(Node *ll1_head, Node *ll2_head) {
    ...
    while (ll1_curr != NULL && ll2_curr != NULL) {
        ...
        // which have repeated values
            ...
            int curr_val = ll1_curr->val;
            while (ll1_curr != NULL && ll1_curr->val == curr_val) ll1_curr = ll1_curr->next;
            printf("\nFinished skipping ll1");
            while (ll2_curr != NULL && ll2_curr->val == curr_val) ll2_curr = ll2_curr->next;
            printf("\nFinished skipping ll2");
            printf("\nAre pointers NULL?: ll1 %d, ll2 %d", ll1_curr == NULL, ll2_curr == NULL);
        }
...
//            ll_arr[i + interval] = NULL;
            printf("\nCurrent merging result:\n");
            print_linked_list(ll_arr[i]);
        }

...
us largest_rect_area(const us *matrix, us size) {
    ...
    for (us i = 0; i < size; i++) {
        ...
        // Check if correct - REMOVE ME
        printf("\n\nHeights array:\n");
        print_arr(heights_arr, size);
        ...
        printf("\nStarted calculating max area for the i=%hd row", i);
...
void print_arr(us arr[], us size) {
    for (us i = 0; i < size; i++) printf("%hd ", arr[i]);
    printf("\n");
}

void print_stack(Stack* stack) {
    for (us i = 0; i < stack->next_idx; i++) {
        printf(" %hd", *((stack->arr) + i));
    }
    printf("\n");
}
